...
class OpenAICLIP(nn.Module):
    def __init__(self, config):
        ...
        if config.clip_image_size == 224:
            model, _ = clip.load("/nas/shared/sport/wuhao/code/diffcap/thirdparty/EVCap/ckpts/OpenAI-ViT-L-14-224.pth", jit=False)
            # model, _ = clip.load("/nas/shared/sport/wuhao/model/ckpts/BAAI/DIVA/OpenAICLIP/OpenAI-ViT-L-14-224.pth", jit=False)
        if config.clip_image_size == 336:
            model, _ = clip.load("/nas/shared/sport/wuhao/model/ckpts/BAAI/DIVA/OpenAICLIP/OpenAI-ViT-L-14-336.pth",jit=False)
    ...
    def forward(self, images):
        ...
        image_features = self.model.encode_image_feats(images).float()
